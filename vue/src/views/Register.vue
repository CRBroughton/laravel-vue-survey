  ...
  <form class="mt-8 space-y-6" @submit.prevent="register">
    ...
    <div class="-space-y-px rounded-md shadow-sm">
      <div>
        ...
        <input
          id="fullname" v-model="user.name" name="name" type="text" autocomplete="name"
          required="true"
      ...
      <div>
        <label for="password" class="sr-only">Password</label>
        <input
          id="password" v-model="user.password_confirmation" name="password_confirmation" type="password" autocomplete="current-password"
          required="true"
          class="relative block w-full appearance-none rounded-none rounded-b-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Password confirmation"
        >
      </div>
    </div>
